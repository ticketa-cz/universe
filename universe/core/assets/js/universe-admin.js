(function($){
	$(document).ready(function(){
			...
			if( $(e.target).hasClass('vc_controls-row') ){
				if( $(e.target).parent().hasClass('wpb_vc_row') ){
					...
					if( row.parent().attr('id') == 'visual_composer_content' ){
						...
						var uri = new RegExp( UNIVERSE_SITE_URI,'g');
						var raw = data.html.replace( regx, function( m , n, i, j, k, content){
							
							return '[vc_raw_html]'+Base64.encode( escape( unescape( Base64.decode( content ) ).replace( uri, '<?php echo UNIVERSE_SITE_URI; ?>') ) )+'[/vc_raw_html]';
							
						});
						raw = raw.replace( uri, '%UNIVERSE_SITE_URI%');
						copy2clipboard( raw );
		...
		$('body').append('<div id="king-theme-sections" class="popup-overlay" style="display: none;"><div class="popup"><div class="popup-head">'+
						'<h3 class="poptit">Install '+UNIVERSE_THEME_NAME+' Theme Sections</h3>'+
...
var sections = {
	...
	loadFromServer : function( page ){
		...
		jQuery.post( UNIVERSE_SITE_URI+'/wp-admin/admin-ajax.php', {
	...
	install : function( data ){
		...
		data = data.replace( /%UNIVERSE_SITE_URI%/g , UNIVERSE_SITE_URI );
