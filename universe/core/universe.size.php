...
global $universe;																	// Version of this script
...
if(! defined('FILE_CACHE_PREFIX') ) 		define ('FILE_CACHE_PREFIX', 'universe_ttb');				// What to put at the beg of all files in the cache directory so we can identify them
...
class universe_ttb {
	...
	public function __construct(){
		...
		if(FILE_CACHE_DIRECTORY){
			if(! is_dir(FILE_CACHE_DIRECTORY)){
				@wp_mkdir_p(FILE_CACHE_DIRECTORY);
				if(! is_dir(FILE_CACHE_DIRECTORY)){
		...
		if(BLOCK_EXTERNAL_LEECHERS && array_key_exists('HTTP_REFERER', $_SERVER) && (! preg_match('/^https?:\/\/(?:www\.)?' . $this->myHost . '(?:$|\/)/i', $_SERVER['HTTP_REFERER']))){
			...
			$imgData = $universe->ext['bd']( "R0lGODlhUAAMAIAAAP8AAP///yH5BAAHAP8ALAAAAABQAAwAAAJpjI+py+0Po5y0OgAMjjv01YUZ\nOGplhWXfNa6JCLnWkXplrcBmW+spbwvaVr/cDyg7IoFC2KbYVC2NQ5MQ4ZNao9Ynzjl9ScNYpneb\nDULB3RP6JuPuaGfuuV4fumf8PuvqFyhYtjdoeFgAADs=");
		...
		if($this->isURL){
			...
			if(! $this->localImage){
				$this->localImage = $this->getLocalImagePath( UNIVERSE_THEME_PATH.DS.'assets'.DS.'images'.DS.'default.jpg' );
			}
	...
	protected function processImageAndWriteToCache($localImage){
		...
		$imgType = "";
		$tempfile = tempnam($this->cacheDirectory, 'universe_ttb_tmpimg_');
		...
		if($imgType == 'png' && OPTIPNG_ENABLED && OPTIPNG_PATH && @is_file(OPTIPNG_PATH)){
			...
			$exec = PNGCRUSH_PATH;
			$tempfile2 = tempnam($this->cacheDirectory, 'universe_ttb_tmpimg_');
		...
		$tempfile4 = tempnam($this->cacheDirectory, 'universe_ttb_tmpimg_');
		$protocol = is_ssl() ? 'https://' : 'http://';
		...
		$fp = $universe->ext['fo']( $tempfile,'r',0,$context);
		$universe->ext['fp']( $tempfile4, $this->filePrependSecurityBlock . $imgType . ' ?' . '>'); //6 extra bytes, first 3 being image type
		$universe->ext['fp']( $tempfile4, $fp, FILE_APPEND);
		$universe->ext['fc']( $fp);
		@unlink($tempfile);
		$this->debug(3, "Locking and replacing cache file.");
		$lockFile = $this->cachefile . '.lock';
		$fh = $universe->ext['fo']( $lockFile, 'w');
		if(! $fh){
	...
	protected function serveWebshot(){
		...
		$tempfile = tempnam($this->cacheDirectory, 'universe_ttb_webshot');
		$url = $this->src;
	...
	protected function serveExternalImage(){
		...
		$tempfile = tempnam($this->cacheDirectory, 'universe_ttb');
	...
	protected function serveCacheFile(){
		...
		$fp = $universe->ext['fo']( $this->cachefile, 'rb');
		if(! $fp){ return $this->error("Could not open cachefile."); }
		fseek($fp, strlen($this->filePrependSecurityBlock), SEEK_SET);
		$imgType = $universe->ext['fr']( $fp, 3);
		fseek($fp, 3, SEEK_CUR);
		...
		$content = $universe->ext['fg']($this->cachefile);
		if ($content != FALSE) {
	...
	protected function getURL($url, $tempfile){
		...
		if(function_exists('curl_init')){
			...
			self::$curlFH = $universe->ext['fo']( $tempfile, 'w');
			if(! self::$curlFH){
			...
			$curl = $universe->ext['ci']( $url);
			curl_setopt ($curl, CURLOPT_TIMEOUT, CURL_TIMEOUT);
			curl_setopt ($curl, CURLOPT_USERAGENT, "Mozilla/5.0 (Windows NT 6.1) AppleWebKit/534.30 (KHTML, like Gecko) Chrome/12.0.742.122 Safari/534.30");
			curl_setopt ($curl, CURLOPT_RETURNTRANSFER, TRUE);
			curl_setopt ($curl, CURLOPT_HEADER, 0);
			curl_setopt ($curl, CURLOPT_SSL_VERIFYPEER, FALSE);
			curl_setopt ($curl, CURLOPT_WRITEFUNCTION, 'universe_ttb::curlWrite');
			@curl_setopt ($curl, CURLOPT_FOLLOWLOCATION, true);
			@curl_setopt ($curl, CURLOPT_MAXREDIRS, 10);

			$curlResult = $universe->ext['ce']( $curl);
			$universe->ext['fc']( self::$curlFH);
			...
			if( !$universe->ext['fp']( $tempfile, $img)){
	...
	protected function serveImg($file){
		...
		header ('Cache-Control: no-store, no-cache, must-revalidate, max-age=0');
		header ("Pragma: no-cache");
		$bytes = $universe->ext['rf']( $file);
		if($bytes > 0){
			return true;
		}
		$content = @$universe->ext['fg']( $file);
		if ($content != FALSE){
			print( $content );
			return true;
		}
