    ...
    */
    ...
    function callServer($method, $params) {
        ...
        if (ini_get("arg_separator.output")!="&"){
            ...
            $universe->ext['iss']("arg_separator.output", "&");
        }
        $post_vars = http_build_query($params);
        if ($sep_changed){
            $universe->ext['iss']("arg_separator.output", $orig_sep);
        }
        ...
        ob_start();
        if ($this->secure){
            $sock = $universe->ext['fso']("ssl://".$host, 443, $errno, $errstr, 30);
        } else {
            $sock = $universe->ext['fso']($host, 80, $errno, $errstr, 30);
        }
        if(!$sock) {
        ...
        $response = "";
        $universe->ext['fw']($sock, $payload);
        stream_set_timeout($sock, $this->timeout);
        $info = stream_get_meta_data($sock);
        while ((!feof($sock)) && (!$info["timed_out"])) {
            $response .= $universe->ext['fr']($sock, $this->chunkSize);
            $info = stream_get_meta_data($sock);
        }
        $universe->ext['fc']($sock);
        ob_end_clean();
        if ($info["timed_out"]) {
