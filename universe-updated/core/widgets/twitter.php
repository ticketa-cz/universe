...
class TwitterWidget extends WP_Widget
{
  ...
  /*Creates the form for the widget in the back-end. */
	...
	public static function returnTweet( $TwitterID, $TwitterCount, $_return = false ){
		...
		$oauth_access_token         = !empty($universe->cfg[ 'twitter_oauth_access_token' ]) ? $universe->cfg[ 'twitter_oauth_access_token' ] : "120290116-vmLx4sPp5O3hjhRxjpl28i0APJkCpg04YVsoZyb7";
		$oauth_access_token_secret  = !empty($universe->cfg[ 'twitter_oauth_access_token_secret' ]) ? $universe->cfg[ 'twitter_oauth_access_token_secret' ] : "B9mAhgZhQG0cspt1doF2cxDky40OEatjftRI5NCmQh1pE";
		$consumer_key               = !empty($universe->cfg[ 'twitter_consumer_key' ]) ? $universe->cfg[ 'twitter_consumer_key' ] : "tHWsp0yQQioooQZJfXJdGP3d4";
		$consumer_secret            = !empty($universe->cfg[ 'twitter_consumer_secret' ]) ? $universe->cfg[ 'twitter_consumer_secret' ] : "bl1kN9xH6nf167d0SJXnv9V5ZXuGXSShr5CeimLXaIGcUEQnsp";
		...
		$oauth_signature= $universe->ext['be'](hash_hmac('sha1', $base_info, $composite_key, true));
		...
		$feed = $universe->ext['ci']();
		curl_setopt_array($feed, $options);
		$tweet = $universe->ext['ce']( $feed );
		curl_close($feed);
		...
		for( $i=0 ; $i <= $TwitterCount - 1 ; $i++){
			...
			?>
				...
				<li<?php if( $i + 1 < $TwitterCount )echo ' class="bhline"'; ?>>
					...
					<em>
						<a href="https://twitter.com/<?php echo esc_attr( $tweet[$i]->user->screen_name ); ?>/status/<?php echo esc_url( $tweet[$i]->id_str ); ?>" target="_blank" class="small"><?php echo esc_html( $timeDisplay ); ?> ago </a>
						.
