...
function universe_like_scripts() {
	wp_enqueue_script( 'universe_like_post', get_template_directory_uri() .'/core/assets/js/post-like.js', array('jquery'), UNIVERSE_THEME_VERSION, 1 );
	wp_localize_script( 'universe_like_post', 'ajax_var', array(
		'url' => admin_url( 'admin-ajax.php' ),
		'nonce' => wp_create_nonce( 'ajax-nonce' )
		)
	);
}
add_action( 'init', 'universe_like_scripts' );

/**
 * (2) Save like data
 */
add_action( 'wp_ajax_nopriv_universe-post-like', 'universe_post_like' );
add_action( 'wp_ajax_universe-post-like', 'universe_post_like' );
function universe_post_like() {
	$nonce = $_POST['nonce'];
    if ( ! wp_verify_nonce( $nonce, 'ajax-nonce' ) )
        die ( 'Nope!' );

	if ( isset( $_POST['universe_post_like'] ) ) {
...
function getPostLikeLink( $post_id ) {
	...
	$count = ( empty( $like_count ) || $like_count == "0" ) ? '<i class="fa fa-thumbs-up"></i><span> 0</span>' : '<i class="fa fa-thumbs-up"></i><span> ' . $like_count . '</span>';
	...
	$output = '<a href="#" class="universe-post-like'.$class.'" data-post_id="'.$post_id.'" title="'.$title.'">'.$count.'</a>';
	return $output;
}
 ...
 */
add_action( 'show_user_profile', 'universe_show_user_likes' );
add_action( 'edit_user_profile', 'universe_show_user_likes' );
function universe_show_user_likes( $user ) { ?>
    <table class="form-table">
        <tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'universe' ); ?></label></th>
			<td>
            <?php
			...
			if ( $count > 0 ) {
				...
				echo universe::esc_js($like_list);
			} else {
				echo "<p>" . _e( 'You don\'t like anything yet.', 'universe' ) . "</p>\n";
			} ?>
            </td>
		</tr>
    </table>
 ...
 * type [hslike] in your post to output the button
 */
function universe_like_shortcode() {
	return getPostLikeLink( get_the_ID() );
}
$universe->ext['asc']('hslike', 'universe_like_shortcode');
...
function frontEndUserLikes() {
	if ( is_user_logged_in() ) { // user is logged in
		...
		if ( $count > 0 ) {
			...
			$like_list .= "<aside>\n";
			$like_list .= "<h3>" . esc_html__( 'You Like:', 'universe' ) . "</h3>\n";
			$like_list .= "<ul>\n";
