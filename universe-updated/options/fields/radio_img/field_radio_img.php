<?php
class universe_options_radio_img extends universe_options{	
	...
	*/
	function render(){
		...
		if(isset($universe->cfg[$this->field['id'].'_profile_selected']))
			$list_selected = $universe->cfg[$this->field['id'].'_profile_selected'];
		...
		echo '<fieldset class="radio-img">';
			...
			foreach($this->field['options'] as $k => $v){
				...
				echo '<img src="'.$v['img'].'" alt="'.$v['title'].'" onclick="jQuery: universe_radio_img_select(\''.$this->field['id'].'_'.array_search($k,array_keys($this->field['options'])).'\', \''.$this->field['id'].'\');" />';
	...
	*/
	function enqueue(){
		...
		wp_enqueue_script(
			...
			universe_options_URL.'fields/radio_img/field_radio_img.js', 
			array('jquery'),
			time(),
			true
		);
