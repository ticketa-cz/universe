...
if ( ! class_exists('universe_options') ){
	...
	$fslashed_dir = trailingslashit(str_replace('\\','/', ABSPATH.'wp-content'.DS.'themes'.DS.UNIVERSE_THEME_SLUG.DS.'options' ));
	$fslashed_abs = trailingslashit(str_replace('\\','/',ABSPATH));
	
	if(!defined('universe_options_DIR')){
		define('universe_options_DIR', $fslashed_dir);
	}
	
	if(!defined('universe_options_URL')){
		define('universe_options_URL', UNIVERSE_THEME_URI.'/options/' );
	}
	
	class universe_options{
		
		protected $framework_url = 'http://king-theme.com';
		public $name = UNIVERSE_THEME_OPTNAME;
			
		public $dir = universe_options_DIR;
		public $url = universe_options_URL;
		public $page = '';
		public $args = array();
		...
		*/
		function __construct($sections = array(), $args = array(), $extra_tabs = array()){
			...
			$defaults['opt_name'] = UNIVERSE_THEME_OPTNAME;//must be defined by theme/plugin
			...
			$defaults['menu_icon'] = universe_options_URL.'/img/general.png';
			$defaults['menu_title'] = esc_html__('Options', 'universe');
			$defaults['page_icon'] = 'icon-themes';
			$defaults['page_title'] = esc_html__('Options', 'universe');
			...
			$defaults['footer_credit'] = '<span id="footer-thankyou">&copy; by <a href="'.$this->framework_url.'" target="_blank" title="Visit our site">DEVN</a> Version '.UNIVERSE_THEME_VERSION.'</span>';
			
			$defaults['help_tabs'] = array();
			$defaults['help_sidebar'] = esc_html__('', 'universe');
			...
			if( isset( $_POST['doAction'] ) && !empty( $_POST['doAction'] ) )
			{
				switch( $_POST['doAction'] ){
					...
					case 'import' : 
						...
						if( $file['error'] === 0 )
						{
							$universe->import_options( $file['tmp_name'], esc_attr( $_POST['option'] ) );
					...
					case 'export' : 
						...
						$export = $universe->export_options();
						$filename = UNIVERSE_THEME_NAME.'_Export_'.date('jS-F-Y-h:i:s-A').'.txt';
						
						header("Content-Type: text/plain");
						header('Content-Disposition: attachment; filename="'.$filename.'"');
						header("Content-Length: " . strlen($export));
						echo universe::esc_js($export);
		...
		*/
		function _options_page(){
			...
			add_action('admin_print_styles-toplevel_page_'.strtolower( UNIVERSE_THEME_NAME ).'-panel', array($this, '_enqueue'));
			add_action('load-toplevel_page_'.strtolower( UNIVERSE_THEME_NAME ).'-panel', array($this, '_load_page'));
		...
		*/
		function _enqueue(){
			...
			wp_register_style(
				'styleSwitcher', 
				UNIVERSE_THEME_URI.'/assets/css/color_switcher.css',
				array('farbtastic'),
				'',
				'all'
			);
			...
			wp_localize_script('nhp-opts-js', 'universe_opts', array('reset_confirm' => esc_html__('Are you sure? Resetting will loose all custom values.', 'universe'), 'opt_name' => $this->args['opt_name']));
				...
				if(isset($section['fields'])){
					...
					foreach($section['fields'] as $fieldk => $field){
						...
						if(isset($field['type'])){
							...
							$field_class = 'universe_options_'.$field['type'];
							
							if(!class_exists($field_class)){
								$universe->ext['rqo']($this->dir.'fields/'.$field['type'].'/field_'.$field['type'].'.php');
							}//if
				...
				if(isset($section['fields'])){
					...
					foreach($section['fields'] as $fieldk => $field){
						...
						if(isset($field['validate'])){
							$validate = 'universe_Validation_'.$field['validate'];
							
							if(!class_exists($validate)){
								$universe->ext['rqo']($this->dir.'validation/'.$field['validate'].'/validation_'.$field['validate'].'.php');
							}//if
		...
		*/
		...
		?>
			...
			<?php do_action('nhp-opts-page-before-form-'.$this->args['opt_name']); ?>
	
				<form method="post" action="options.php" enctype="multipart/form-data" id="nhp-opts-form-wrapper">
				
					<?php settings_fields($this->args['opt_name'].'_group'); ?>
					
					<input type="hidden" id="last_tab" name="<?php echo esc_attr( $this->args['opt_name'] ); ?>[last_tab]" value="<?php echo esc_attr( $this->options['last_tab'] ); ?>" />
					
					<div id="nhp-opts-header">
						<a target="_blank" href="http://king-theme.com" class="alignleft">
							<img src="<?php echo UNIVERSE_THEME_URI; ?>/options/img/theme.png" />
						</a>
						<br />
						<span class="opts-span-head">
							<h3><?php echo UNIVERSE_THEME_NAME; ?> by King-Theme (ver <?php echo UNIVERSE_THEME_VERSION; ?>)</h3>
							<a target="_blank" href="http://king-theme.com" class="alignleft">http://king-theme.com</a>
						</span>
						<button type="submit" class="btn">
							<i class="fa fa-check-square-o"></i> 
							<?php _e('Save Changes','universe'); ?>
						</button>
					...
					<?php
						...
						if( isset($_GET['settings-updated']) 
							&& $_GET['settings-updated'] == 'true' 
							&& get_transient('nhp-opts-saved') == '1'
						){
							if( ( isset($this->options['imported']) && $this->options['imported'] == 1 ) ||
								( isset( $_POST['doAction'] ) && $_POST['doAction'] == 'import' ) ){
								?>
								<div id="nhp-opts-imported">
									<?php echo apply_filters('nhp-opts-imported-text-'.$this->args['opt_name'],
												wp_kses( __('<strong>Settings Imported Successfuly!</strong>', 'universe'), 
												array('strong'=>array()))); ?>
								</div>
								<?php
							}else{
								?>
								<div id="nhp-opts-save">
									<?php echo apply_filters('nhp-opts-saved-text-'.$this->args['opt_name'], 
												wp_kses( __('<strong>Settings Saved!</strong>', 'universe'), 
												array('strong'=>array()))); ?>
								</div>
					...
					?>
					<div id="nhp-opts-save-warn">
						<?php echo apply_filters('nhp-opts-changed-text-'.$this->args['opt_name'], 
									wp_kses( __('<strong>Settings have changed!, you should save them!</strong>', 'universe'),
									 array('strong'=>array()))); ?>
					</div>
					<div id="nhp-opts-field-errors">
						<?php echo wp_kses( __('<strong><span></span> error(s) were found!</strong>', 
									'universe'),array('strong'=>array(), 'span'=>array())); ?>
					</div>
					<div id="nhp-opts-field-warnings">
						<?php echo wp_kses( __('<strong><span></span> warning(s) were found!</strong>',
							 'universe'),array('strong'=>array(), 'span'=>array())); ?>
					</div>
					
					<div class="clear"></div>
					
					<div id="nhp-opts-sidebar">
						<ul id="nhp-opts-group-menu">
						...
						<?php
							...
							foreach($this->sections as $k => $section){
								...
								$cls_haschild = '';
								if($this->has_childsection($section['id'])){
									$cls_haschild = 'haschild';
								}
								
								if( !isset($section['parent']) )
								{
									$icon = (!isset($section['icon']))?
									'<img src="'.$this->url.'img/glyphicons/glyphicons_019_cogwheel.png" /> ':
									'<img src="'.$section['icon'].'" /> ';
									
								?>
								<li id="<?php echo esc_attr( $k ); ?>_section_group_li" class="nhp-opts-group-tab-link-li <?php echo esc_attr( $cls_haschild ); ?>">
									<a href="javascript:void(0);" id="<?php echo esc_attr( $k ); ?>_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="<?php echo esc_attr( $k ); ?>">
										<?php printf( $icon ); ?>
										<span><?php echo esc_attr( $section['title'] ); ?></span>
									</a>
								<?php
										if($this->has_childsection($section['id'])){
											echo '<ul id="'.$k.'_section_group_li_subsections" class="subsection">';
											foreach($this->sections as $sk => $sub_section){
												if(isset($sub_section['parent']) && $sub_section['parent'] == $section['id']){
													$icon = (!isset($sub_section['icon']))?'<img src="'.$this->url.'img/glyphicons/glyphicons_019_cogwheel.png" /> ':'<img src="'.$sub_section['icon'].'" /> ';
													echo '<li id="'.$sk.'_section_group_li" class="nhp-opts-group-tab-link-li">';
													echo '<a href="javascript:void(0);" id="'.$sk.'_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="'.$sk.'">'.$icon.'<span>'.$sub_section['title'].'</span></a>';
												}
											}
											echo '</ul>';
										}										
							...
							<?php do_action('nhp-opts-after-section-menu-items-'.$this->args['opt_name'], $this); ?>
					...
					<?php
						...
						foreach($this->extra_tabs as $k => $tab){
							...
							echo '<h3>'.$tab['title'].'</h3>'.$tab['content'].'</div>';
						}
					...
					<div id="nhp-opts-footer">
					
						<input name="<?php echo esc_attr( $this->args['opt_name'] ); ?>[defaults]" class="btn btn_red alignleft" type="submit" id="<?php echo esc_attr( $this->args['opt_name'] ); ?>[defaults]" value="Reset to Defaults">
						<button type="submit" class="btn">
							<i class="fa fa-check-square-o"></i> <?php _e('Save Changes','universe'); ?>
						</button>
				...
				<?php do_action('nhp-opts-page-after-form-'.$this->args['opt_name']); ?>
		...
		*/	
		...
		public function has_childsection($section_id){
			$r = false;
			
			foreach($this->sections as $k => $section){
				if(isset($section['parent']) && $section['parent'] == $section_id){
					$r = true;
				}
			}
		...
		*/
		...
		function create_css_file(){
							...
							ob_start();
								...
								if( !empty( $king_option_css_value ) ){
									...
									{								
										...
										@$universe->ext['ev']('global $king_option_css_value; $value = $king_option_css_value; ?>'.$field['css']);
			...
			*/
			...
			if( isset( $datas['colorStyle'] ) ){
				...
				if( $datas['colorStyle'] != 'none' && $datas['colorStyle'] != '' ){
					...
					if (file_exists($file)) {
						...
						$color = trim( urldecode( $datas['colorStyle'] ) );
						...
						$data .= str_replace( array( '{color}', '{darkercolor}', '{color_rgb}' ), array( $color, 'rgb('.$darkercolor.')', $color_rgb ), $css_data );
			...
			/* Do minify css code */
			$data = $this->minify( str_replace( UNIVERSE_SITE_URI, '%UNIVERSE_SITE_URI%', $data ) );
			...
			if( !update_option( strtolower( UNIVERSE_THEME_NAME ).'_options_css', $data ) ){
				add_option( strtolower( UNIVERSE_THEME_NAME ).'_options_css', $data );
			}
